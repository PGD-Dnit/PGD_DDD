...
namespace PGD.UI.Mvc.Controllers
{
    public class PactoController : BaseController
    {
        ...
        readonly IUsuarioPerfilUnidadeService _UsuarioPerfilUnidadeService;
...
#pragma warning disable S107 // Methods should not have too many parameters
        public PactoController(ILogService logService, IUsuarioAppService usuarioAppService, IPactoAppService pactoservice, IUnidadeService unidadeService, IUsuarioPerfilUnidadeService UsuarioPerfilUnidadeService,
...
#pragma warning restore S107 // Methods should not have too many parameters
        ...
        private void ConfigurarNomesServidoresPesquisa()
        {
            ...
            if (userLogado.IsDirigente)
            {
                ...
                foreach (var unid in unidades)
                {
                    //csa parei aqui falta limpar codigo
                    ...
                    //   // inativo = (int?)domain.enums.status.ativo,
                    //})?.lista ?? new list<usuarioviewmodel>();
        ...
        private void ConfigurarGruposAtividades(OrdemServicoViewModel osvm, int idTipoPacto)
        {
            if (idTipoPacto == 1)
            {
                ...
                //TempData["GruposAtividades"] = _grupoAtividadeService.ObterTodos();
            }
            else
            {
        ...
        private void AtualizarReferenciasProduto(ProdutoViewModel model, OrdemServicoViewModel ordemVigente)
        {
            ...
            var faixa = atividade.Tipos.FirstOrDefault(x => x.IdTipoAtividade == model.IdTipoAtividade);// talvez aqui acrescentar o idmodalidade p defini faixa ou faixa presencial##########
            var qtdFaixa = atividade.Tipos.Count();
            var situacaoProduto = model.IdSituacaoProduto;
            var modalidade = model.IdModalidade;
        ...
        public PartialViewResult PreparaAlteracaoProduto(int indexProduto, int idPacto, int idTipoPacto)
        {
            ...
            //csa p/ alterar o botão cadastrar p editar
            pvm._BtnEdit = true;
